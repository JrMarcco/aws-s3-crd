...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "126c3259.jrmarcco.io",
	...
	if !ok {
		setupLog.Error(errors.New("load aws access key id failed"), "unable to load environment")
	}
	key, ok := os.LookupEnv("AWS_SECRET_ACCESS_KEY")
	if !ok {
		setupLog.Error(errors.New("load aws secret access key failed"), "unable to load environment")
	}
	region, ok := os.LookupEnv("REGION")
	if !ok {
		setupLog.Info("use default aws region")
